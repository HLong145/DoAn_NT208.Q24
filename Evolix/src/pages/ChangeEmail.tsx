...
import { changeEmail, getAuthSession, saveAuthSession } from '../services/authApi';
...
export default function ChangeEmail() {
  ...
  const currentEmail = getAuthSession()?.user.email ?? '';
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    try {
      const response = await changeEmail(newEmail, currentPassword);

      const session = getAuthSession();
      if (session) {
        saveAuthSession({ ...session, user: { ...session.user, email: response.email } });
      }
  ...
  return (
    <main className="flex-1 min-w-0 border-r border-border pb-20 sm:pb-0 relative">
      <div className="sticky top-0 bg-bg-base/85 backdrop-blur-xl z-10 border-b border-border p-3 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-[20px] font-extrabold tracking-tight">Change email</h1>
          <p className="text-[13px] text-text-muted">Update your email address</p>
        </div>
      </div>

      <div className="max-w-[620px] p-4 sm:p-6">
        {error && <div className="mb-4 text-sm text-red-500">{error}</div>}
        {success && <div className="mb-4 text-sm text-primary">Email updated successfully.</div>}

        <p className="text-sm text-text-muted mb-4">
          Current email: <span className="text-text-base font-medium">{currentEmail}</span>
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-2xl border border-border overflow-hidden">
            <div className="px-4 py-3 border-b border-border bg-bg-panel">
              <label className="text-sm text-text-muted block mb-1">New email address</label>
              <input
                type="email"
                value={newEmail}
